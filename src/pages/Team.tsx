  ...
  return (
    <div id="team-page-root" className="bg-slate-50 dark:bg-slate-950 min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16 space-y-16">
        ...
        {/* Leadership Grid */}
        <AnimatedCardContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
          {team.map((member) => (
            <AnimatedCard key={member.id}>
              <div 
                id={`team-member-profile-${member.id}`}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all flex flex-col justify-between h-full"
              >
                <div>
                  {/* Member Picture */}
